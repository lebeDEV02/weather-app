...
document.addEventListener('DOMContentLoaded', (e) => {
	...
	for (let key in localStorage) {
		if (!localStorage.hasOwnProperty(key) || key === "inputStorage") {
			continue;
		}
	...
	for (let value of set) {
		addedLocations.insertAdjacentHTML('beforeend', `<p class="app__favourite-city">${value}
		 <svg class="app__favourite-icon" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M16 9v10H8V9h8m-1.5-6h-5l-1 1H5v2h14V4h-3.5l-1-1zM18 7H6v12c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7z"/></svg>
		 </p>`);
	};
})
// Sort out keys in localStorage
favouriteCity.addEventListener('click', () => {
	if (weatherCityFavourite.textContent !== "undefined") {
		counterForLocalStorage++;
		localStorage.setItem(`${counterForLocalStorage}`, `${weatherCityFavourite.textContent}`);
		alert(localStorage.getItem('1'));
		alert(counterForLocalStorage);
	}
	for (let key in localStorage) {
		if (!localStorage.hasOwnProperty(key) || key === "inputStorage") {
			continue;
